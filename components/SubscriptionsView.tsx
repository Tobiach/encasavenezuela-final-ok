    ...
    <div className="fixed inset-0 z-[300] bg-black/60 backdrop-blur-sm flex items-end md:items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
          <div>
            <p className="text-[10px] font-black text-[#8B1A1A] uppercase tracking-widest mb-0.5">Suscripción</p>
            <h3 className="font-black text-[#1F2937] text-base">{plan.emoji} {plan.name}</h3>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {/* Precio */}
          <div className="flex items-baseline gap-2 bg-[#FFD700]/10 border border-[#FFD700]/30 rounded-2xl px-4 py-3">
            <span className="text-2xl font-black text-[#1F2937]">${plan.price.toLocaleString('es-AR')}</span>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
              <div>
                ...
                <p className="text-sm font-black text-[#1F2937]">{ALIAS}</p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
              <div>
                ...
                <p className="text-[11px] font-black text-[#1F2937] tracking-wider">{CBU}</p>
              </div>
              <button
          ...
          <div>
            ...
            <button
              ...
              className={`w-full border-2 border-dashed rounded-2xl py-4 flex flex-col items-center gap-2 transition-all ${
                file ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-[#FFD700]/60 hover:bg-[#FFD700]/5'
              }`}
            >
              {file ? (
                <>
          ...
          {sent ? (
            ...
            <button
              onClick={handleSend}
              className="w-full bg-[#8B1A1A] text-white font-black py-4 rounded-2xl flex items-center justify-center gap-2 text-sm active:scale-[0.98] transition-all shadow-lg shadow-[#8B1A1A]/25"
            >
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] pb-28">
      ...
      <div className="bg-white border-b border-gray-100 px-6 pt-10 pb-8 text-center">
        <div className="inline-flex items-center gap-2 bg-[#FFD700]/15 border border-[#FFD700]/30 text-[#8B1A1A] font-black text-[9px] uppercase tracking-[0.25em] px-4 py-1.5 rounded-full mb-4">
          <Zap size={10} fill="currentColor" /> Planes Mensuales
        </div>
        <h1 className="text-3xl md:text-4xl font-black text-[#1F2937] uppercase tracking-tight leading-tight mb-3">
          Suscripciones<br /><span className="text-[#FFD700]" style={{ WebkitTextStroke: '1px #D4AF37' } as React.CSSProperties}>Premium</span>
        </h1>
          ...
          return (
            <div
              ...
              className={`bg-white rounded-3xl border-2 p-5 shadow-sm transition-all ${
                plan.highlight ? 'border-[#FFD700]/60 shadow-[#FFD700]/15 shadow-md' : 'border-gray-100'
              }`}
            >
              {plan.highlight && (
                <div className="inline-flex items-center gap-1.5 bg-[#FFD700] text-black text-[8px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-3 shadow-sm">
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className="text-lg font-black text-[#1F2937] leading-tight">{plan.emoji} {plan.name}</h3>
                ...
                {store && (
                  <div className="flex items-center gap-1 bg-gray-100 rounded-xl px-2 py-1">
                    <Store size={10} className="text-[#FFD700] shrink-0" />
              ...
              <div className="bg-gray-50 rounded-2xl p-3.5 space-y-2 mb-4 border border-gray-100">
                {plan.items.map((item, i) => (
                  <div key={i} className="flex items-center gap-2 text-[11px] font-semibold text-gray-700">
                    <div className="w-4 h-4 bg-[#FFD700]/20 rounded-full flex items-center justify-center shrink-0">
                      <Zap size={8} className="text-[#8B1A1A]" fill="currentColor" />
                    </div>
                    {item}
                  </div>
                ))}
              ...
              <button
                ...
                className="w-full bg-[#8B1A1A] text-white font-black py-3.5 rounded-2xl text-sm flex items-center justify-center gap-2 active:scale-[0.98] transition-all shadow-lg shadow-[#8B1A1A]/20 hover:brightness-110"
              >
